...
define( 'KANTANBOND_VERSION', '1.4.26' );
...
require_once KANTANBOND_PLUGIN_DIR . 'includes/class-shortcode-align.php';
require_once KANTANBOND_PLUGIN_DIR . 'includes/class-frontend-assets.php';
...
require_once KANTANBOND_PLUGIN_DIR . 'includes/class-reference.php';
 ...
 * [kantanbond_billing_plans unlock="（合言葉）"]
 * サイト全体で合言葉なしにする場合のみ:
...
function kantanbond_run(): void {
	...
	$billing_plans   = new KantanBond_Billing_Plans( $settings, $api );
	$reference       = new KantanBond_Reference( $api );
	$admin           = new KantanBond_Admin( $settings, $logger, $api );
	$loader          = new KantanBond_Loader( $admin, $shortcodes, $public_products, $purchase_thank_you, $billing_plans, $reference );
